...
try:
    ...
    # Sidebar untuk filter (Opsional, tapi bagus untuk interaktivitas)
    st.sidebar.header("Filter Tampilan")
    show_table = st.sidebar.checkbox("Tampilkan Tabel Data Asli", value=True)
    ...
    if komoditas_plot:
        ...
        selected_commodities = st.multiselect(
            ...
            default=komoditas_plot  # Default tampilkan semua target
        )

        if selected_commodities:
            fig, ax = plt.subplots(figsize=(14, 7))
            # Gunakan loop dari selected_commodities
